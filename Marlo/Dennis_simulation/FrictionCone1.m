...
nlp.addConstraint(0, Fe1(2), 100); % Positive vertical force must be positive
...
% nlp.addConstraint(-1, G2(3,:)*xdot2.final, 1);
...
% nlp.addConstraint(0, g2(1), 0);
% nlp.addConstraint(0, g2(2), 0);
...
[g, ~, ~] = sys.constraintEquation(x1, xdot1, 'ds');  % Get position of swing leg toe
...
% nlp.addConstraint(toeProfile, g(4), 0.20); % Apply ground clearance constraint
...
% Stance foot force (non-negative)
% nlp.addConstraint(0, F2(2), 1200); % Positive vertical force
% % Reformulate absolute value constraint as two linear inequalities
