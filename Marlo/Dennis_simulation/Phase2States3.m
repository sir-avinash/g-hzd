...
nlp.addConstraint(-1e-6,g1_final(4),1e-6);
...
% nlp.addConstraint(-1e-6, d1 - (x1(1).final - x1(1).initial), 1e-6); % Constraint distance to be travel of hip during one step
% d2 = nlp.addVariable(0, -Inf, Inf); % Slack variable for distance
% nlp.addConstraint(-1e-6, d2 - (x2(1).final - x2(1).initial), 1e-6); % Constraint distance to be travel of hip during one step
