function [p4L,p4,EL,ER,ELq,ERq]= LagrangeModelAtriasConstraint2D(q,dq)
...
%%%%
ELq=zeros(2,1)*q(1);
ELq(1,1)=L2*dq2L^2*sin(q2L + qT) + L4*dq1L^2*sin(q1L + qT) + L2*...
         dqT^2*sin(q2L + qT) + L4*dqT^2*sin(q1L + qT) + 2*L2*dq2L*dqT*...
         sin(q2L + qT) + 2*L4*dq1L*dqT*sin(q1L + qT);
ELq(2,1)=- L2*dq2L^2*cos(q2L + qT) - L4*dq1L^2*...
         cos(q1L + qT) - L2*dqT^2*cos(q2L + qT) - L4*dqT^2*...
         cos(q1L + qT) - 2*L2*dq2L*dqT*cos(q2L + qT) - 2*L4*dq1L*dqT*...
         cos(q1L + qT);
%%%%
%%%%
ERq=zeros(2,1)*q(1);
...
%%%%
p4L=zeros(2,1)*q(1);
p4L(1,1)=yH - L2*sin(q2L + qT) - L4*sin(q1L + qT);
p4L(2,1)=zH + L2*cos(q2L + qT) + L4*cos(q1L + qT);
return