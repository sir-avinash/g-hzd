...
load Response16-Jan-2017_day_l0_50_l1_50_periodic.mat
tic
...
% end
% controller.CBF_ld(1)=0.4; controller.CBF_hd(1)=0.15;
controller.CBF_ld=...
[0.4000    0.3353    0.7002    0.3718    0.6578    0.5166    0.7804    0.6745    0.6234    0.3094...
0.4812    0.6901    0.3668    0.5799    0.7697    0.4433    0.7481    0.7420    0.5746    0.5884];

controller.CBF_hd=...
[0.1500    0.2536   -0.2284    0.2909    0.2034   -0.2176   -0.1385    0.2023   -0.1154   -0.2147...
0.2729    0.2011   -0.2871   -0.1195    0.2885    0.1805    -0.2585    0.2662    0.2370   -0.2845];
% controller.CBF_ld=[0.7];
% controller.CBF_hd=[0.15];
...
stepHeight = -0.5*ones(1,length(stepLength));

stepHeight(stepLength<=controller.stone_size)=0;
% stepHeight(stepLength<=-controller.stone_size)=-0.5;
% stepHeight((stepLength<=-0.4+controller.stone_size)&(stepLength>=-0.4-controller.stone_size))=-0.25;
...
stepHeight(stepLength>=stone_x(end)-controller.stone_size)=stone_y(end);
shape = 'nearest';
...
%% controller set up
% PD controller
controller.Kp = 80*20*eye(4)/6; %8000/50;%
controller.Kd = 20*5*eye(4)/6; %300/50;%
% A = [zeros(4) eye(4);
...
scene = AtriasSceneColor(sys, response);
% 
% shape='previous';
