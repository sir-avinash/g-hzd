...
classdef PendulumCartSystemSymbolic < SecondOrderSystem
...
% 		xLim@double scalar = 10 % Cart position limits
	...
	% PUBLIC METHODS ========================================================
	methods
		function obj = PendulumCartSystemSymbolic(nLinks)
...
% 			g = sym('g');
			...
			for i = 2:nLinks
				xp = xp - (lp - rp)*sin(thetat{i-1}) - rp*sin(thetat{i});
				yp = yp + (lp - rp)*cos(thetat{i-1}) + rp*cos(thetat{i});
				U = U + mp*g*yp;
		...
		%SECONDORDERSTATEEQUATION The system state equation.
			...
			% Inertia matrix
			M = obj.MFcn(x, xdot, u);	% theta == 0, up right
