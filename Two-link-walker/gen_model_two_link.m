%gen_model_two_link.m
%
%   Generate model for two-link biped walker and output to
...
if isempty(dir([filename,'.mat']))
	...
	%
	syms th1 th2 z1 z2 real
	syms dth1 dth2 dz1 dz2 real
	syms m Mh g L r real
	syms th1d
	...
	%
	Ha=[th2-(-th1+(a02+a12*th1+a22*th1^2+a32*th1^3)*(th1-th1d)*(th1+th1d))];
...
%% Read in constants
%
fprintf(fid,'[r,m,Mh,g]=model_params_two_link;\n\n');
fprintf(fid,'[th1d,alpha,epsilon]=control_params_two_link;\n\n');

%% Reassign configuration parameters
%
fprintf(fid,'th1=x(1); th2=x(2); \n');
fprintf(fid,'dth1=x(3); dth2=x(4); \n\n');
...
fprintf(fid,'a02=a(1); a12=a(2); a22=a(3); a32=a(4);\n\n');
...
fprintf(fid,'x_new(5)=tmp_vec(5);\n');
fprintf(fid,'x_new(6)=tmp_vec(6);\n');
fprintf(fid,'z2_new=tmp_vec(4);\n');
fclose(fid);
...
ttt=char(solve(Ha(1),th2));
fprintf(fid,'th2 = %s;\n',ttt);
ttt=char(solve(LfHa(1),dth2));
fprintf(fid,'dth2 = %s;\n',ttt);
% ttt=char(solve(LfHa(1),dth3));
% fprintf(fid,'dth3 = %s;\n\n',ttt);

fprintf(fid,'x = [th1,-th1,dth1,dth2];');
fclose(fid);