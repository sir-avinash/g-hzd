...
[D,C,G,B,K,dV,dVl,Al,Bl,H,LfH,dLfH] = dynamics_two_link(x(1:4),a);
Fx = inv(D)*(-C*x(3:4)-G);
Gx = inv(D)*B;
...
u = inv(dLfH*[zeros(2,1);Gx])*(v-dLfH*[x(3:4);Fx]);
...
t_2 = [t_2 ; t];
y = [y ; H.'];
[f_tan,f_norm] = stance_force_two_link(x(1:4),dx(1:4),u);
...
% plot(time_local,force_local(:,2))
    ...
    function cost = objfun(a)
        x_init = sigma_two_link(a);
        x0 = transition_two_link(x_init).';
        x0 = x0(1:4);
        ...
        cost = sum((trapz(t,u)).^2) ; %sum(torque(:,1)'*torque(:,1) + torque(:,2)'*torque(:,2));
        cost
    end    
...
for i = 1:5 % run five steps
	...
	x0=transition_two_link(x(nt,:));

	% display some useful information to the user
	disp(['step: ',num2str(i),', impact ratio:  ',num2str(x0(5)/x0(6))])
...
q=xe(1,1:3);
[pFoot1,pFoot2,pH]=limb_position(q,pH_horiz(1));
...
for k=2:n
	q=xe(k,1:3);
	[pFoot1,pFoot2,pH]=limb_position(q,pH_horiz(k));
...
function [pFoot1,pFoot2,pH] = limb_position(q,pH_horiz)
